...
@Service("com.yugandhar.mdm.match.rules.LeCorporationDeterministicMatchRule")
public class LeCorporationDeterministicMatchRule {
	...
	public LeCorporationDeterministicMatchRule() {
	...
	public TxnTransferObj process(TxnTransferObj txnTransferObj) throws YugandharCommonException {
		...
		try {
			...
			if (logger.isInfoEnabled()) {
				logger.info("LeCorporationDeterministicMatchRule.process - Started Matching rule for TxnMessageId:"
			...
			ConfigAppPropertiesDO theConfigAppPropertiesDO = configAppPropertiesComponent.executeRepositoryQuery(
					YugandharConfigurationProperties.com_yugandhar_match_le_Deterministic_LeCorporation_inquiryLevel_default,
			...
			if (logger.isInfoEnabled()) {
				logger.info("LeCorporationDeterministicMatchRule.process - Finished Matching rule for TxnMessageId:"
						+ txnTransferObj.getTxnHeader().getTxnMessageId() + " reqLegalentityIdpk:"
						+ txnTransferObj.getTxnPayload().getPerformLeMatchRequestDO().getLegalentityDO().getIdPk()
						+ " detailed logs are captured in match engine logs");
			}
	...
	private String matchCorporationLE(LegalentityDO reqLegalentityDO, LegalentityDO probableCandidateLegalentityDO,
		...
		if (matchEngineLogger.isInfoEnabled()) {
			matchEngineLogger.info("LeCorporationDeterministicMatchRule.matchCorporationLE for TxnMessageId:"
					+ txnTransferObj.getTxnHeader().getTxnMessageId() + " reqLegalentityIdpk:"
		...
		if (null != reqLegalentityDO.getLeCorporationDO().getCorporationnamesDOList()
			...
			if (matchEngineLogger.isInfoEnabled()) {
				matchEngineLogger.info("LeCorporationDeterministicMatchRule.performMatching for TxnMessageId:"
						+ txnTransferObj.getTxnHeader().getTxnMessageId() + " reqLegalentityIdpk:"
						+ reqLegalentityDO.getIdPk() + " Candidate LegalEntityIdpk:"
		...
		for (LegalentityDO probableCandidateLegalentityDO : probableCandidateLegalentityDOList) {
			...
			if (null == respRefMatchScoreDO) {
				...
				if (matchEngineLogger.isInfoEnabled()) {
					...
					matchEngineLogger
							.info("LeCorporationDeterministicMatchRule.performMatching result action  for TxnMessageId:"
									+ txnTransferObj.getTxnHeader().getTxnMessageId() + " reqLegalentityIdpk:"
									+ reqLegalentityDO.getIdPk() + " probableCandidateLegalentityDO: "
	...
	private SearchMatchCandidateLERequestDO createRequestSearchMatchCandidateLEDO(TxnTransferObj txnTransferObj) {
		...
		if (matchEngineLogger.isInfoEnabled()) {
			matchEngineLogger
					.info("LeCorporationDeterministicMatchRule.createRequestSearchMatchCandidateLE creating request SearchMatchCandidateLE for reqLegalentityIdpk:"
							+ reqLegalentityDO.getIdPk());
		}
		// Retrieve Corporationnames
		if (null != reqLegalentityDO.getLeCorporationDO().getCorporationnamesDOList()
			...
			if (matchEngineLogger.isInfoEnabled()) {
				matchEngineLogger
						.info("LeCorporationDeterministicMatchRule.createRequestSearchMatchCandidateLE - reqLegalentityIdpk:"
								+ reqLegalentityDO.getIdPk() + " corporationNameList : -" + corporationNameList);
			}

		}

		// Retrieve LeIdentifierKycRegistry
		if (null != reqLegalentityDO.getLeIdentifierKycRegistryDOList()
	...
	public void addToMatchCandidateLeRegistry(String legalentityIdpk, String candidateLegalentityidpk,
		...
		if (matchEngineLogger.isInfoEnabled()) {
			...
			matchEngineLogger
					.info(" LeCorporationDeterministicMatchRule.addToMatchCandidateLeRegistry - legalentityIdpk:"
		...
		theMatchCandidateLeRegistryDO.setMatchPercentageDescription("LeCorporationDeterministicMatchRule");
