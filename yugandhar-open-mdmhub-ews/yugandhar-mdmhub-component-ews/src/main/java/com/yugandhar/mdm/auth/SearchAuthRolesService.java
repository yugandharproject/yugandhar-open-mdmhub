...
@Service("com.yugandhar.mdm.auth.SearchAuthRolesService")
public class SearchAuthRolesService {
	...
	public TxnTransferObj process(TxnTransferObj txnTransferObj) throws YugandharCommonException {
		...
		try {
			...
			List<AuthRolesRegistryDO> searchResultAuthRolesRegistryDOList = searchQuery.getResultList();

			respTxnPayload.setAuthRolesRegistryDOList(searchResultAuthRolesRegistryDOList);
	...
	private Query buildSearchQuery(TxnTransferObj txnTransferObj) {
		...
		//		"SELECT DISTINCT AUTH_ROLES_REGISTRY.ID_PK, AUTH_ROLES_REGISTRY.VERSION, AUTH_ROLES_REGISTRY.CREATED_TS, AUTH_ROLES_REGISTRY.DELETED_TS, AUTH_ROLES_REGISTRY.UPDATED_TS, AUTH_ROLES_REGISTRY.UPDATED_BY_USER, AUTH_ROLES_REGISTRY.UPDATED_BY_TXN_ID, AUTH_ROLES_REGISTRY.ROLE_NAME, AUTH_ROLES_REGISTRY.DESCRIPTION FROM AUTH_ROLES_REGISTRY ");
		...
		if (inquiryFilter.equals(yugandharConstants.FILTER_VALUE_ACTIVE)) {
			queryCriteriaString.append(
					" WHERE (AUTH_ROLES_REGISTRY.DELETED_TS IS NULL OR AUTH_ROLES_REGISTRY.DELETED_TS > CURRENT_TIMESTAMP) ");

			if (!isNullOrEmpty(userName)) {
				queryCriteriaString.append(
						" AND (AUTH_USER_REGISTRY.DELETED_TS IS NULL OR AUTH_USER_REGISTRY.DELETED_TS > CURRENT_TIMESTAMP) ");
			...
			queryCriteriaString.append(
					" WHERE (AUTH_ROLES_REGISTRY.DELETED_TS IS NOT NULL AND AUTH_ROLES_REGISTRY.DELETED_TS < CURRENT_TIMESTAMP) ");

			if (!isNullOrEmpty(userName)) {
				queryCriteriaString.append(
						" AND (AUTH_USER_REGISTRY.DELETED_TS IS NOT NULL AND AUTH_USER_REGISTRY.DELETED_TS < CURRENT_TIMESTAMP) ");
		...
		if (!isNullOrEmpty(userName)) {
			...
			queryJoinString.append(
					" JOIN AUTH_USER_ROLE_ASSOC ON AUTH_USER_ROLE_ASSOC.AUTH_ROLES_REGISTRY_IDPK = AUTH_ROLES_REGISTRY.ID_PK ");
			queryJoinString.append(
					" JOIN AUTH_USER_REGISTRY ON AUTH_USER_REGISTRY.ID_PK = AUTH_USER_ROLE_ASSOC.AUTH_USER_REGISTRY_IDPK ");
		...
		logger.info("SearchAuthRolesService search Query is -" + queryJoinString.toString());
		// get Native query instance
		// Query searchQuery =
		// entityManager.createNativeQuery(queryJoinString.toString());
		Query searchQuery = entityManager.createNativeQuery(queryJoinString.toString(), AuthRolesRegistryDO.class);
		...
		for (Iterator<Entry<String, String>> iterator = paramMap.entrySet().iterator(); iterator.hasNext();) {
			...
			logger.debug(
					"SearchAuthRolesService parameter Name:" + mapEntry.getKey() + " Value:" + mapEntry.getValue());
