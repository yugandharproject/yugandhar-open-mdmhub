...
@Service("com.yugandhar.mdm.match.rules.LePersonFuzzyMatchRule")
public class LePersonFuzzyMatchRule {
	...
	public TxnTransferObj process(TxnTransferObj txnTransferObj) throws YugandharCommonException {
		...
		try {
			...
			if (logger.isInfoEnabled()) {
				logger.info("LePersonFuzzyMatchRule.process - Started Matching rule for TxnMessageId:"
			...
			ConfigAppPropertiesDO theConfigAppPropertiesDO = configAppPropertiesComponent.executeRepositoryQuery(
					YugandharConfigurationProperties.com_yugandhar_match_le_Fuzzy_LePerson_inquiryLevel_default,
			...
			SearchMatchCandidateLERequestDO theSearchMatchCandidateLERequestDO = createRequestSearchMatchCandidateLEDO(
					txnTransferObj);
			theSearchMatchCandidateLERequestDO.setPhoneticSearch(true);
			...
			if (logger.isInfoEnabled()) {
				logger.info("LePersonFuzzyMatchRule.process - Finished Matching rule for TxnMessageId:"
						+ txnTransferObj.getTxnHeader().getTxnMessageId() + " reqLegalentityIdpk:"
						+ txnTransferObj.getTxnPayload().getPerformLeMatchRequestDO().getLegalentityDO().getIdPk()
						+ " detailed logs are captured in match engine logs");
			}
	...
	private String matchPersonLE(LegalentityDO reqLegalentityDO, LegalentityDO probableCandidateLegalentityDO,
		...
		if (matchEngineLogger.isInfoEnabled()) {
			matchEngineLogger.info("LePersonFuzzyMatchRule.matchPersonLE for TxnMessageId:"
					+ txnTransferObj.getTxnHeader().getTxnMessageId() + " reqLegalentityIdpk:"
		...
		if (!(theYugandharMatchingUtils.isNullOrEmpty(reqLegalentityDO.getLePersonDO().getGenderRefkey())
			...
			if (matchEngineLogger.isInfoEnabled()) {
				matchEngineLogger.info("LePersonFuzzyMatchRule.performMatching for TxnMessageId:"
						+ txnTransferObj.getTxnHeader().getTxnMessageId() + " reqLegalentityIdpk:"
						+ reqLegalentityDO.getIdPk() + " Candidate LegalEntityIdpk:"
	...
	private void performMatching(LegalentityDO reqLegalentityDO, List<LegalentityDO> probableCandidateLegalentityDOList,
		...
		for (LegalentityDO probableCandidateLegalentityDO : probableCandidateLegalentityDOList) {
			...
			if (null == respRefMatchScoreDO) {
				...
				if (matchEngineLogger.isInfoEnabled()) {
					matchEngineLogger
							.info("LePersonFuzzyMatchRule.performMatching result action  for TxnMessageId:"
									+ txnTransferObj.getTxnHeader().getTxnMessageId() + " reqLegalentityIdpk:"
				...
				matchEngineLogger.info("LePersonFuzzyMatchRule.performMatching result action  for TxnMessageId:"
						+ txnTransferObj.getTxnHeader().getTxnMessageId() + " reqLegalentityIdpk:"
						+ reqLegalentityDO.getIdPk() + " probableCandidateLegalentityDO: "
			...
			if (null == respRefMatchScoreDO) {
				...
				if (matchEngineLogger.isInfoEnabled()) {
					matchEngineLogger
							.info("LePersonFuzzyMatchRule.performMatching - No action as RefMatchScoreDO not found for attributeMatchPattern "
									+ attributeMatchPattern);
				}
	...
	private SearchMatchCandidateLERequestDO createRequestSearchMatchCandidateLEDO(TxnTransferObj txnTransferObj) {
		...
		if (matchEngineLogger.isInfoEnabled()) {
			matchEngineLogger
					.info("LePersonFuzzyMatchRule.createRequestSearchMatchCandidateLE creating request SearchMatchCandidateLE for reqLegalentityIdpk:"
		...
		if (null != reqLegalentityDO.getLePersonDO().getPersonnamesDOList()
			...
			if (matchEngineLogger.isInfoEnabled()) {
				matchEngineLogger
						.info("LePersonFuzzyMatchRule.createRequestSearchMatchCandidateLE - reqLegalentityIdpk:"
	...
	public void addToMatchCandidateLeRegistry(String legalentityIdpk, String candidateLegalentityidpk,
		...
		if (matchEngineLogger.isInfoEnabled()) {
			...
			matchEngineLogger.info(" LePersonFuzzyMatchRule.addToMatchCandidateLeRegistry - legalentityIdpk:"
		...
		theMatchCandidateLeRegistryDO.setMatchPercentageDescription("LePersonFuzzyMatchRule");
